...
fun main() {
    ...
    class Guard(var x: Int, var y: Int, private val map: Grid) {
        ...
        fun move(): Boolean {
            if (dir == 'U') {
                var nextX = x - 1
                var nextY = y
                if (!map.isValid(Position(nextX, nextY))) {
                    return false
                }
    ...
    fun part2(input: List<String>): Int {
        ...
        for (position in path) {
            ...
            if (checkIfLoop(Guard(initialGuardPosition.x, initialGuardPosition.y, Grid(modifiedMap)))) {
                totalWithLoop++
            }
        }
